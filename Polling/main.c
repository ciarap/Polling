...
int main(void)
{
 ...
 // PORT1->IOCR0 = PORT1->IOCR0 | ( (PORT1_IOCR0_PC0_Msk & 0x10) << PORT1_IOCR0_PC0_Pos );
  ...
  while(1U)
  {
	  	  ...
	  	  //PWM_pin = PORT2->IN & PORT2_IN_P0_Msk;     		// non-zero result if P2.0 = 1, else zero.

         PWM_pin= DIGITAL_IO_GetInput(&EX_SWITCH);
		   ...
		   if ( !DIGITAL_IO_GetInput(&EX_SWITCH) )
	  		  while ( !(PWM_pin=DIGITAL_IO_GetInput(&EX_SWITCH)) );		// loop until next edge transition
	  	  else
	  	  {													// P2.0 currently one, but may be mid-cycle!
	  		  while ( DIGITAL_IO_GetInput(&EX_SWITCH));		// loop while still one
	  	      while ( !(PWM_pin=DIGITAL_IO_GetInput(&EX_SWITCH)));		// now loop while zero until next rising edge
	  	  }

	  	  // Use "count" variable as rough indication of signal high period.

	  	  count = 0;
	  	  while ( DIGITAL_IO_GetInput(&EX_SWITCH) ) count++;	// keep counting while P2.0 = 1
